# ZARATI | زرعتي — Hardened Migration 024 Specification & DDL Design
**Phase:** R4-A (Institutional & Historical Data Foundation)  
**Status:** PROPOSED HARDENED DESIGN ONLY (NOT APPLIED / NOT CREATED)  
**Target Migration File:** `20260907000024_024_r4_a_data_foundation.sql` (UNCREATED)  
**Database:** PostgreSQL 15+ / Supabase  
**Base Schema:** Migrations 001–023 (Immutable)

> **STATUTORY NOTICE:**  
> This document specifies the EXACT Hardened Data Definition Language (DDL), security policies, immutability triggers, and safe views for Migration 024.  
> **NO MIGRATION FILE HAS BEEN CREATED. NO DATABASE MUTATION HAS OCCURRED. NO CODE WAS MODIFIED.**  
> Execution of this schema remains strictly blocked pending Founder review and Implementation Gate sign-off.

---

## 1. Architectural Strategy & Hardening Core Principles

### 1.1 Verbatim Source Truth Preservation
Migration 024 enforces absolute preservation of original raw evidence:
* `raw_price_text TEXT NOT NULL`: Exact source string (e.g. `'450,000'`, `'12.5'`, `'1500-1600'`). Never mutated.
* `parsed_price_numeric NUMERIC(16,4)`: Parsed interpretation, nullable until parsing succeeds. Parsing is a transformation, not ground truth.
* `raw_currency_text TEXT NOT NULL` & `raw_unit_text TEXT NOT NULL`: Preserved verbatim.

### 1.2 Strict R4-A / R4-C Subsystem Boundary
R4-C owns unit normalization and foreign exchange conversion. R4-A **does not require normalized values** to store a valid observation.
* Normalized columns (`normalized_price_per_mt`, `normalized_price_per_kg`, `normalized_price_usd_per_mt`, `conversion_factor_used`, `conversion_rule_applied`) are **strictly NULLABLE** in R4-A.
* Any future normalization is recorded in `observation_transformations` as an auditable versioned transformation without altering raw source fields.

### 1.3 Orthogonal Provenance Model
Separates orthogonal concepts previously conflated into a single enum:
* `source_provenance_enum`: Institutional identity (`sovereign_statutory`, `institutional_multilateral`, `commercial_exchange`, `market_reported`, `field_survey`).
* `ingestion_method_enum`: Operational ingestion mechanism (`automated_api`, `automated_feed`, `manual_bulletin_entry`, `ocr_extract`, `batch_import`).
* `temporal_class_enum`: Time horizon (`current_spot`, `historical_archive`, `backfill`).
* `derivation_class_enum`: Epistemic nature (`observed_transaction`, `reported_survey`, `calculated_median`, `model_estimated`).

### 1.4 Multi-Dimensional Verification Evidence
Replaces the single `verification_dimension` column with `observation_verification_evidence`. Multiple dimensions (`SOURCE_AUTHORITY`, `INGESTION_INTEGRITY`, `FIELD_CORROBORATION`, `STATISTICAL_PLAUSIBILITY`) can be independently satisfied with auditable evidentiary references. Statistical plausibility is never treated as factual verification.

### 1.5 Zero Default Trust
`trust_score` is nullable (`CHAR(1) DEFAULT NULL`). Verification state initializes to `'unassessed'`. New records never inherit synthetic trust badges.

### 1.6 Public Data Minimization & View Security
* **Direct Access Revoked:** Public anonymous and authenticated users have **ZERO** `SELECT` access to `market_price_observations`, `raw_ingestion_snapshots`, `observation_quality_flags`, `observation_conflict_ledger`, `observation_verification_evidence`, or `observation_transformations`.
* **Safe Public View:** `v_approved_market_prices` is configured with `security_barrier = true`, exposing strictly non-sensitive, sanitized metrics exclusively for rows where `publication_status = 'PUBLISHED'`.

### 1.7 Deterministic Deduplication & Relational Consistency
* `source_record_key TEXT NOT NULL`: Deterministic SHA-256 hash derived from immutable source fields ensuring idempotent ingestion even when upstream source record IDs are missing or non-unique.
* Relational enforcement: Composite foreign key `FOREIGN KEY (dataset_id, source_id) REFERENCES canonical_datasets(id, source_id)` guarantees an observation cannot link a dataset belonging to Source A while claiming Source B.

### 1.8 Deterministic Staleness & Immutability Triggers
* Replaces mutable `is_stale BOOLEAN` with immutable `stale_after_at TIMESTAMPTZ NOT NULL`. The safe view evaluates `(NOW() > stale_after_at) AS is_stale` dynamically.
* Immutability trigger (`trg_protect_mpo_immutability`) forbids updates to any raw source or provenance fields.
* Deletion trigger (`trg_prevent_mpo_delete`) forbids physical `DELETE` on observations. Retraction preserves historical audit rows.
* State transition trigger (`trg_enforce_mpo_publication_state`) enforces strict workflow gates.

---

## 2. Hardened Enumerated Types (Enums)
...
CREATE TABLE IF NOT EXISTS public.canonical_commodity_aliases (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  source_id UUID NOT NULL REFERENCES public.canonical_sources(id) ON DELETE CASCADE,
...
## 4. Ingestion Ledgers & Observation Model
...
## 5. Verification Evidence, Transformations, Quality & Conflict Ledgers
...
CREATE TABLE IF NOT EXISTS public.observation_conflict_ledger (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  primary_observation_id UUID NOT NULL REFERENCES public.market_price_observations(id) ON DELETE CASCADE,
  conflicting_observation_id UUID NOT NULL REFERENCES public.market_price_observations(id) ON DELETE CASCADE,
...
## 6. Immutability, Governance & State Machine Triggers
...
## 7. Security Architecture, RLS & Safe Public Views
...
```sql
-- ROLLBACK DDL (ONLY EXECUTABLE IN LOCAL DEV / PRE-PRODUCTION TESTBED)
DROP VIEW IF EXISTS public.v_legacy_crop_prices_bridge CASCADE;
DROP VIEW IF EXISTS public.v_approved_market_prices CASCADE;

DROP TRIGGER IF EXISTS trg_enforce_mpo_publication_state ON public.market_price_observations;
DROP FUNCTION IF EXISTS public.fn_enforce_mpo_publication_state();

DROP TRIGGER IF EXISTS trg_prevent_mpo_delete ON public.market_price_observations;
DROP FUNCTION IF EXISTS public.fn_prevent_market_price_observations_delete();

DROP TRIGGER IF EXISTS trg_protect_mpo_immutability ON public.market_price_observations;
DROP FUNCTION IF EXISTS public.fn_protect_market_price_observations_immutability();

DROP TABLE IF EXISTS public.observation_conflict_ledger CASCADE;
DROP TABLE IF EXISTS public.observation_quality_flags CASCADE;
DROP TABLE IF EXISTS public.observation_transformations CASCADE;
DROP TABLE IF EXISTS public.observation_verification_evidence CASCADE;
DROP TABLE IF EXISTS public.market_price_observations CASCADE;
DROP TABLE IF EXISTS public.raw_ingestion_snapshots CASCADE;
DROP TABLE IF EXISTS public.canonical_market_aliases CASCADE;
DROP TABLE IF EXISTS public.canonical_commodity_aliases CASCADE;
DROP TABLE IF EXISTS public.canonical_units CASCADE;
DROP TABLE IF EXISTS public.canonical_commodities CASCADE;
DROP TABLE IF EXISTS public.canonical_schemes CASCADE;
DROP TABLE IF EXISTS public.canonical_localities CASCADE;
DROP TABLE IF EXISTS public.canonical_datasets CASCADE;
DROP TABLE IF EXISTS public.canonical_sources CASCADE;
DROP TABLE IF EXISTS public.canonical_currencies CASCADE;

DROP TYPE IF EXISTS public.verification_dimension_enum CASCADE;
DROP TYPE IF EXISTS public.source_tier_enum CASCADE;
DROP TYPE IF EXISTS public.publication_status_enum CASCADE;
DROP TYPE IF EXISTS public.verification_state_enum CASCADE;
DROP TYPE IF EXISTS public.derivation_class_enum CASCADE;
DROP TYPE IF EXISTS public.temporal_class_enum CASCADE;
DROP TYPE IF EXISTS public.ingestion_method_enum CASCADE;
DROP TYPE IF EXISTS public.source_provenance_enum CASCADE;
DROP TYPE IF EXISTS public.temporal_precision_enum CASCADE;
