...
export function SudanMap({ lang, className = '' }: SudanMapProps) {
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="absolute top-4 right-4 z-10 flex flex-col gap-2.5 font-mono text-[10px] tracking-widest text-text/70 uppercase">
        <div className="flex items-center gap-2 justify-end">
          {isAr ? 'المروي' : 'IRRIGATED'}
          <div className="w-4 h-0.5 bg-success"></div>
        </div>
        <div className="flex items-center gap-2 justify-end">
          {isAr ? 'المطري الآلي' : 'MECHANIZED'}
          <div className="w-4 h-0.5 bg-warning"></div>
        </div>
        <div className="flex items-center gap-2 justify-end">
          {isAr ? 'النيل' : 'NILE SYSTEM'}
      ...
      >
        <defs>
          <pattern id="grid" width="20" height="20" patternUnits="userSpaceOnUse">
            <path d="M 20 0 L 0 0 0 20" fill="none" stroke="currentColor" strokeWidth="0.5" className="text-border-strong opacity-40" />
          </pattern>
        </defs>

        <rect width="100%" height="100%" fill="url(#grid)" className="opacity-70" />

        <text x="10" y="440" className="fill-muted font-mono text-[8px] opacity-70">15.5007° N, 32.5599° E</text>
        <text x="440" y="20" className="fill-muted font-mono text-[8px] opacity-70">SUDAN / 2026</text>
        ...
        <path d={NILE_WHITE}  stroke="var(--color-blue)" strokeWidth="1.5" fill="none" strokeLinecap="round" opacity="0.6" />
        <path d={NILE_BLUE}   stroke="var(--color-blue)" strokeWidth="1.5" fill="none" strokeLinecap="round" opacity="0.8" />
        <path d={NILE_MAIN}   stroke="var(--color-blue)" strokeWidth="2" fill="none" strokeLinecap="round" opacity="0.8" />
        <path d={NILE_ATBARA} stroke="var(--color-blue)" strokeWidth="1" fill="none" strokeLinecap="round" opacity="0.4" strokeDasharray="3 2" />
        <path d={KHOR_GASH}   stroke="var(--color-teal)" strokeWidth="1" fill="none" strokeLinecap="round" opacity="0.4" strokeDasharray="1 3" />

        {CITIES.map((c) => {
          const flipLabel = c.x > 390
          const lx = flipLabel ? c.x - 6 : c.x + 6
          const anchor = flipLabel ? 'end' : 'start'
          return (
            <g key={c.id}>
              {c.capital && (
                <circle cx={c.x} cy={c.y} r={8}
                  fill="none" stroke="var(--color-primary)" strokeWidth="0.5" opacity="0.3" className="animate-pulse" />
              )}
              <circle cx={c.x} cy={c.y} r={c.capital ? 3 : 2}
                fill={c.capital ? 'var(--color-primary)' : 'var(--color-muted)'} />
              <text x={lx} y={c.y + 2} textAnchor={anchor} fill="var(--color-text)"
                fontSize={c.capital ? 7 : 6} fontFamily="sans-serif"
                fontWeight={c.capital ? '700' : '500'} opacity="0.9">
