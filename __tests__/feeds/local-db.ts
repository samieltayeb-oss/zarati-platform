...
export async function ready(feed: 'WFP' | 'OPEN_METEO' | 'MET_NORWAY'): Promise<Lease> { await sql.query("UPDATE public.feed_leases SET next_allowed_at='-infinity' WHERE feed_type=$1", [feed]); const r = await db.rpc('acquire_feed', { p_feed: feed }); if (r.error)
