...
SELECT f.feed_type,
 ...
 CASE WHEN f.feed_type::text IN ('OPEN_METEO','MET_NORWAY') THEN (SELECT jsonb_build_object('evaluated_at',now(),'latest_valid_time',max(valid_time),'fresh_current',count(*) FILTER(WHERE temporal_class='CURRENT_MODEL_ESTIMATE' AND NOT is_stale),'future_fresh_forecasts',count(*) FILTER(WHERE temporal_class='FORECAST' AND NOT is_stale AND valid_time>=now()),'stale_rows',count(*) FILTER(WHERE is_stale)) FROM public.v_public_weather WHERE provider=f.feed_type::text) END AS weather_freshness
FROM unnest(ARRAY['WFP'::public.feed_type,'OPEN_METEO'::public.feed_type,'MET_NORWAY'::public.feed_type]) f(feed_type);
ROLLBACK;
